...
Это форк EssentialsX для серверов Bukkit/Paper. Устанавливайте JAR в папку `plugins` нужного сервера.
...
При `teleport-delay` больше нуля в чат один раз выводится обычное сообщение EssentialsX о предстоящей телепортации. Отсчёт обновляется каждую секунду только в action bar; `teleport-countdown-action-bar: false` отключает его. Старое поле `teleport-countdown-chat` больше не используется. Сообщения `/bc`, `/ping` и отсчёта добавлены во все языковые ресурсы. Если включён `per-player-locale`, игрок получает формулировку на своём языке.
...
`/clear` и `/clearinventory` без аргументов очищают инвентарь вместе с бронёй. При очистке конкретного игрока аргумент `*` оставляет броню, а `**` удаляет её. Сборка не регистрирует перечисленные выше удалённые команды EssentialsX. Команды с теми же именами могут предоставлять сервер или другие плагины.
...
После успешной сборки push в `2.x` создаётся предварительный GitHub Release. Его можно выпустить и вручную через `workflow_dispatch` на ветке `2.x`. В релизе лежат отдельные JAR-файлы всех девяти модулей, общий архив `EssentialsX-all-jars.zip` и `SHA256SUMS.txt`. Тег имеет вид `build-<номер запуска>-<короткий хеш>`. Сборка веток `dev/*` и pull request не публикует релиз.
